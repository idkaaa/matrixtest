...
void SingleThreadMatrixMultiplication(int &matrix_size);
void MultipleThreadMatrixMultiplication(int &matrix_size);
...
int main(void)
{
	...
	int matrix_sizes[3] = { 40, 1000, 4000 };
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		matrixLog(message);
		SingleThreadMatrixMultiplication(matrix_size);
		++matrix_size;
	}
